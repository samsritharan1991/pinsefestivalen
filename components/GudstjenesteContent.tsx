...
export default function GudstjenesteContent({ service }: Props) {
  ...
  const renderItem = (item: ServiceItem, idx: number) => {
    ...
    const linkStyle = { fontWeight: "500", textDecoration: "none", color: color };
    const boxStyle = { marginBottom: "0.8em", padding: "0.75em", backgroundColor: "#f9f5f0", borderLeft: `4px solid ${color}`, display: "flex", alignItems: "center", gap: "0.75em" };

    switch (item.type) {
      case "heading":
        return (
          <div key={`heading-${idx}`} style={boxStyle}>
      ...
      case "text":
        ...
        return (
          <div key={`text-${idx}`} style={{ ...boxStyle, marginLeft: isIndentedText ? "1.5em" : "0" }}>
            <span style={{ color: color, fontSize: "1.2em", flexShrink: 0 }}>{icon}</span>
            <Link href={`/programpunkt/${item.slug}`} style={linkStyle}>
              {item.content}
            </Link>
      ...
      case "list":
        return (
          <div key={`list-${idx}`} style={boxStyle}>
            <span style={{ color: color, fontSize: "1.2em", flexShrink: 0 }}>{icon}</span>
      ...
      case "song":
        return (
          <div key={`song-${idx}`} style={boxStyle}>
            <span style={{ color: color, fontSize: "1.2em", flexShrink: 0 }}>{icon}</span>
            <Link href={`/programpunkt/${item.slug}`} style={linkStyle}>
